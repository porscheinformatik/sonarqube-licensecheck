  ...
  <div class="boxed-group boxed-group-inner">
    ...
    <div>
      <table class="data zebra">
        <caption>
          This is a list of all project specific licenses
        </caption>
        <thead>
          <tr>
            <th @click="sort('projectName')" scope="col">
              Project
              <div
                class="arrow"
                v-if="sortBy === 'projectName'"
                v-bind:class="{
        ...
        <tbody>
          <tr v-for="item in displayedItems" :key="item.key">
            ...
            <td>
              <span
                :class="{
                  'icon-license-ok': item.allowed === 'true',
                  'icon-license-nok': item.allowed !== 'true',
                }"
              ></span>
    ...
    <modal-dialog header="Delete License" :show="!!itemToDelete" @close="cancelDelete()">
      <div slot="body" v-if="itemToDelete">
        Are you sure you want to delete the license mapping &quot;{{
          findProjectName(itemToDelete.projectKey)
  ...
  methods: {
    ...
    saveItem(item) {
      if (this.editMode === "add") {
        ...
        const itemToChange = this.items.find(
    ...
    deleteItem(item) {
      this.saveItems(
        this.items.filter((i) => i.projectKey !== item.projectKey || i.license !== item.license),
      );
    },
    sort(param) {
